...
function copy_all () {
  ...
  do
    cd $WORKING_DIR/packages/apps
    cp -r ${array[i]} $WORKING_DIR/LemonadeStore/themes-resources/packages/apps/
  done
}

declare -a root=('LemonadeStore' 'abi' 'bionic' 'art' 'bootable' 'build' 'dalvik' 'development' 'device' 'external' 'frameworks' 'hardware' 'kernel' 'libcore' 'libnativehelper' 'manifest' 'ndk' 'out' 'packages' 'pdk' 'prebuilts'
...
declare -a theme_packages=('Calculator' 'Calendar' 'CalendarWidget' 'Camera2' 'Contacts' 'ContactsCommon' 'DUI' 'Custom-Squash' 'DeskClock' 'Dialer' 'Gallery2'
...
git clone https://github.com/Citrus-CAF/LemonadeStore.git
...
cd values
delete_useless ${values[@]}
rm -rf  $WORKING_DIR/LemonadeStore/themes-resources/frameworks
cd $WORKING_DIR
process_all ${theme_packages[@]}
echo "Cleaning target folders"
rm -rf LemonadeStore/themes-resources/packages
mkdir LemonadeStore/themes-resources/packages
mkdir LemonadeStore/themes-resources/packages/apps
mkdir LemonadeStore/themes-resources/frameworks
